...
const spinnerStyles = `
  .custom-spinner {
    ...
    height: 32px;
    border: 4px solid rgba(255, 255, 255, 0.1);
    border-top: 4px solid #ACBDAA; /* updated color */
    border-radius: 50%;
    animation: spin 1s linear infinite;
  }
  @keyframes spin {
...
const History = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["quizHistory", user?.token, currentPage],
    queryFn: async () => {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/user/quiz-history`, {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (quizId) => {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/user/quiz-history/${quizId}`,
        { method: "DELETE", headers: { "Content-Type": "application/json", Authorization: `Bearer ${user.token}` } }
      );
      if (!response.ok) throw new Error("Failed to delete quiz result");
      return response.json();
    },
    onSuccess: () => {
      toast.success("Quiz result deleted successfully");
  ...
  const getScoreColor = (percentage) => percentage >= 80 ? "text-[#ACBDAA]" : percentage >= 60 ? "text-yellow-400" : "text-red-400";
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <main className="flex-1 p-6 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center min-h-full space-y-4">
              <div className="text-[#ACBDAA] text-xl">No quiz history available</div>
              <Button onClick={() => navigate("/user/dashboard")} variant="outline" className="border-[#ACBDAA] text-[#ACBDAA]/80 hover:text-[#ACBDAA] hover:bg-[#ACBDAA]/10">
                Back to Dashboard
              </Button>
            </div>
          ) : (
            <div className="max-w-4xl mx-auto space-y-6">
              <div className="text-center space-y-2 pt-8">
                <h1 className="text-4xl font-bold text-[#ACBDAA] mb-2">Quiz History</h1>
                <p className="text-[#ACBDAA]/70">Review, retake, or delete your past quiz attempts!</p>
              </div>

              <Card className="bg-[#1E2D4C]/80 border-[#ACBDAA]/30 backdrop-blur-sm shadow-lg shadow-[#ACBDAA]/20">
                <CardHeader>
                  <CardTitle className="text-[#ACBDAA] flex items-center gap-2">
                    <Trophy className="text-yellow-400" />
                    Past Quizzes
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  {paginatedResults.map((result) => {
                    const percentage = (result.score / result.total) * 100;
                    return (
                      <div key={result.quizId} className="p-4 bg-[#1E2D4C]/40 rounded-lg border border-[#ACBDAA]/20 flex items-center justify-between">
                        <div className="flex flex-col gap-2">
                          <div className={`text-lg font-bold ${getScoreColor(percentage)}`}>{result.score}/{result.total} ({percentage.toFixed(1)}%)</div>
                          <div className="text-[#ACBDAA]/80 flex items-center gap-2"><Clock className="w-4 h-4" />{formatTime(result.duration)}</div>
                          <div className="text-[#ACBDAA]/80">{result.level}</div>
                          <div className="text-[#ACBDAA]/80">{result.quizTitle}</div>
                          <div className="text-[#ACBDAA]/50 text-sm">{formatDate(result.submittedAt)}</div>
                          <div className="text-[#ACBDAA]/80">{result.motivationalMessage}</div>
                        </div>

                        <div className="flex gap-2">
                          <Button onClick={() => handleRetakeQuiz(result.quizId, result.quizTitle)} className="bg-[#ACBDAA]/80 text-[#1E2D4C] hover:bg-[#ACBDAA] hover:text-[#1E2D4C]">
                            <RotateCcw className="mr-2 h-4 w-4" /> Retake
                          </Button>
                          <Button onClick={() => handleDeleteQuiz(result.quizId, result.quizTitle)} className="bg-red-600 hover:bg-red-700 text-white" disabled={deleteMutation.isLoading}>
              ...
              {totalPages > 1 && (
                <div className="flex justify-center gap-2 pb-8">
                  <Button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} variant="outline" className="border-[#ACBDAA] text-[#ACBDAA]/70 hover:bg-[#ACBDAA]/10 hover:text-[#ACBDAA]">Previous</Button>
                  <span className="text-[#ACBDAA] self-center">Page {currentPage} of {totalPages}</span>
                  <Button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} variant="outline" className="border-[#ACBDAA] text-[#ACBDAA]/70 hover:bg-[#ACBDAA]/10 hover:text-[#ACBDAA]">Next</Button>
                </div>
              )}

              <div className="flex justify-center pb-8">
                <Button onClick={() => navigate("/user/dashboard")} variant="outline" className="border-[#ACBDAA] text-[#ACBDAA]/70 hover:bg-[#ACBDAA]/10 hover:text-[#ACBDAA]">Back to Dashboard</Button>
              </div>
            </div>
          )}
        </main>
